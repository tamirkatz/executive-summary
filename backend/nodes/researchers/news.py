...
class NewsScanner(BaseResearcher):
    ...
    async def analyze(self, state: ResearchState) -> Dict[str, Any]:
        ...
        if generated_queries:
            # Filter queries relevant to news analysis
            news_queries = [
                query for query in generated_queries 
                if any(keyword in query.lower() for keyword in [
                    'news', 'announcement', 'press release', 'media coverage', 'recent',
                    'latest', 'breaking', 'update', 'development', 'launch', 'partnership',
                    'acquisition', 'merger', 'expansion', 'funding', 'ipo'
                ])
            ]
            
            # If we have news-specific queries, use them
            if news_queries:
                queries = news_queries[:10]  # Limit to 10 queries
            else:
                # Fallback to generating queries if no news queries found
