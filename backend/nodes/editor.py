...
class Editor:
    ...
    async def compile_briefings(self, state: ResearchState) -> ResearchState:
        ...
        if not individual_briefings:
            ...
            logger.warning("No briefings found in state, attempting to create fallback report")
            
            # Try to create a fallback report from available data
            fallback_report = await self.create_fallback_report(state, company)
            if fallback_report:
                state['report'] = fallback_report
                if 'editor' not in state or not isinstance(state['editor'], dict):
                    state['editor'] = {}
                state['editor']['report'] = fallback_report
                logger.info(f"Created fallback report with {len(fallback_report)} characters")
                msg.append(f"Created fallback report from available data")
            else:
                logger.error("Failed to create fallback report")
                msg.append("Failed to create fallback report")
        else:
            try:
...
2. Remove information that is not relevant to {company}, the company headquartered in {hq_location}.
...
Return the cleaned report in flawless markdown format. No explanations or commentary."""
    ...
    async def create_fallback_report(self, state: ResearchState, company: str) -> str:
        ...
        try:
            logger.info(f"Creating fallback report for {company}")
            
            # Collect available data from state
            available_data = []
            ...
            for data_type in ['financial_data', 'news_data', 'company_data']:
                ...
                if curated_data := state.get(curated_key):
                    available_data.append(f"## {data_type.replace('_', ' ').title()}")
                    for url, doc in list(curated_data.items())[:3]:  # Limit to 3 docs per type
                        title = doc.get('title', 'Unknown')
                        content = doc.get('content', '')[:500]  # Limit content length
                        available_data.append(f"### {title}")
                        available_data.append(f"{content}...")
                    available_data.append("")
            
            # Check for site scrape data
            if site_scrape := state.get('site_scrape'):
                available_data.append("## Company Website")
                available_data.append(f"### {site_scrape.get('title', 'Company Information')}")
                content = site_scrape.get('raw_content', '')[:1000]
                available_data.append(f"{content}...")
                available_data.append("")
            
            if not available_data:
                logger.warning("No data available for fallback report")
                return f"# {company} Research Report\n\n## Overview\nUnable to generate comprehensive report due to missing research data. Please try again."
            
            # Create a simple structured report
            report_content = f"# {company} Research Report\n\n"
            report_content += "\n".join(available_data)
            
            # Add references if available
            if references := state.get('references'):
                report_content += "\n## References\n"
                for i, ref in enumerate(references[:10], 1):  # Limit to 10 references
                    report_content += f"{i}. {ref}\n"
            
            logger.info(f"Created fallback report with {len(report_content)} characters")
            return report_content
            
        except Exception as e:
            logger.error(f"Error creating fallback report: {e}")
            return f"# {company} Research Report\n\n## Overview\nError occurred during report generation. Please try again."
